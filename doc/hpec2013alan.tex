...
We believe first time Julia users can readily understand the function {\tt ringmatmul}.
It takes four arguments, the first two, {\tt pl} and {\tt ti}, are functions that will play the role of plus and times.  Functions are first class variables in Julia and thus may be arguments to other functions.  The second two variables are the arrays {\tt A} and {\tt B}.
...
Here we create semiring Integers ``srInt'' directly in a Julia session, overload
``+'' and ``*'' to be hardwired as max and plus on srInt's (and only srInt's).  
\begin{verbatim}
type srInt <: Integer
    k::Int
    srInt(k) = new(k)
end
+(a::srInt, b::srInt) = srInt(max(a.k,b.k))
*(a::srInt, b::srInt) = srInt(a.k+b.k)
\end{verbatim}

A few extra lines come in handy for overloading and io:
\begin{verbatim}
convert(::Type{srInt}, i::Int) = srInt(i)
promote_rule(::Type{srInt}, ::Type{Int}) = srInt
show(io::IO, k::srInt) = print(io, k.k)
\end{verbatim}

Now finally we can convert some ordinary matrices to srInt's and check
that semiring matmul magically works:

\begin{verbatim}
A=map(srInt,rand(1:10,5,5))
B=map(srInt,rand(1:10,5,5))
A*B
\end{verbatim}
...
\bibitem{kepnergilbertbook}  HELLO WORLD
