...
compute-intensive code in a more productive language as well. Instead,
there is pressure to write
...
We note users can now equally well type the infix notation,
{\tt (max * +)(A,B) } though users should avoid potentially ambiguous situations such as
{\tt (+ * *)}.

2) Creation of semiring objects:
This method is recommended for users who are working exclusively in one semiring and wish to optimize notation and performance.  In this method,
...
{\tt A*B} provides the semiring matrix multiply without any notational
hindrance.
...
The function takes four arguments, the first two, {\tt pl} and {\tt ti}, are functions that will play the role of plus and times.  Functions are first class values in Julia and thus may be arguments to other functions.  The second two arguments are the arrays {\tt A} and {\tt B}.
The function {\tt ringmatmul} redefines matrix multiply in terms of {\tt pl} and {\tt ti}.
